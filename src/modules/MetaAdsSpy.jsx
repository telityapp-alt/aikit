...
export default function MetaAdsSpy() {
  ...
  async function submitRun(event) {
    ...
    try {
      ...
      toast.success("Run Meta Ads Spy dimulai. Dashboard akan terisi otomatis.");
      await loadReports();
  ...
  async function downloadReport() {
    ...
    try {
      await api.downloadMetaAdsReport(reportDetail.report.id, (reportDetail.report.query || "report").slice(0, 30));
      ...
      <aside className="tpi-sidebar">
        <div className="tpi-sidebar-head">
          ...
          <p className="tpi-subtitle">
            Bedah iklan kompetitor di Meta Ads Library (Facebook + Instagram) — creative gallery,
            ad copy, format & platform mix, longevity, dan influencer partnerships.
          </p>
        </div>
        ...
        <form className="tpi-launcher" onSubmit={submitRun}>
          ...
          <div className="tpi-grid-two">
            <label className="tpi-field">
              ...
              <input
                value={form.country}
                onChange={(e) => setForm((c) => ({ ...c, country: e.target.value.toUpperCase() }))}
            ...
            <label className="tpi-field">
              ...
              <input
                ...
                max="500"
                value={form.count}
                onChange={(e) => setForm((c) => ({ ...c, count: Number(e.target.value || 10) }))}
              />
            </label>
          </div>
            ...
            {reports.map((entry) => (
              ...
              >
                ...
                <div className="tpi-history-meta">
                  <span>{entry.summary?.totalAds ?? entry.filters?.count ?? 0} ads · {entry.country}</span>
      ...
      <main className="tpi-main">
        {!reportDetail && (
          <div className="tpi-empty-stage">
            ...
            <h2>Run pertama akan membuat Meta ads intelligence deck secara otomatis</h2>
            <p>
              Begitu run berjalan, workspace ini mengisi creative gallery, ad copy swipe file,
              format & platform mix, advertiser leaderboard, longevity, dan influencer partnerships.
            </p>
          </div>
        )}
        ...
        {reportDetail && (
          <>
            <section className="tpi-command">
              <div>
                <span className="tpi-kicker">Meta Ads Deck · {report.country}</span>
                <h2>{report.query}</h2>
                <p>
                  {summary.totalAds ?? reportDetail.items?.length ?? 0} ads ·{" "}
                  market size ~{compactNumber(summary.marketSize)} · {report.filters?.activeStatus || "all"}
                </p>
              </div>
            ...
            <section className="tpi-signal-strip">
              ...
              <article className="tpi-signal-card"><span>Market size</span><strong>{compactNumber(summary.marketSize)}</strong></article>
              <article className="tpi-signal-card"><span>Long-runners</span><strong>{numberFormat(summary.longRunners)}</strong></article>
              <article className="tpi-signal-card"><span>Avg variants</span><strong>{Number(summary.avgVariants || 0).toFixed(1)}</strong></article>
            </section>

            <section className="tpi-kpi-strip">
              <article className="tpi-kpi-card"><span>Avg days active</span><strong>{numberFormat(summary.avgDaysActive)}</strong><em>durasi flight</em></article>
              <article className="tpi-kpi-card"><span>Median days active</span><strong>{numberFormat(summary.medianDaysActive)}</strong><em>benchmark longevity</em></article>
              <article className="tpi-kpi-card"><span>Partnerships</span><strong>{numberFormat(summary.partnershipCount)}</strong><em>influencer terdeteksi</em></article>
              <article className="tpi-kpi-card"><span>Top format</span><strong className="tpi-kpi-text">{summary.formatMix?.[0]?.format || "-"}</strong><em>{summary.formatMix?.[0]?.count || 0} ads</em></article>
              <article className="tpi-kpi-card"><span>Top platform</span><strong className="tpi-kpi-text">{summary.platformMix?.[0]?.platform || "-"}</strong><em>{summary.platformMix?.[0]?.count || 0} ads</em></article>
              <article className="tpi-kpi-card"><span>Top advertiser</span><strong className="tpi-kpi-text">{summary.advertiserLeaderboard?.[0]?.advertiser || "-"}</strong><em>{summary.advertiserLeaderboard?.[0]?.count || 0} ads</em></article>
            </section>

            <section className="tpi-grid-main">
              <div className="tpi-panel">
                <div className="tpi-panel-head"><h3>Longevity (top ads)</h3><span>hari aktif</span></div>
                <BarChart data={(summary.longevitySeries || []).map((d) => ({ label: d.label, value: d.value }))} formatValue={(v) => `${v}d`} />
              </div>
              <div className="tpi-panel">
                <div className="tpi-panel-head"><h3>Advertiser leaderboard</h3><span>by ad volume</span></div>
                <BarChart data={(summary.advertiserLeaderboard || []).slice(0, 8).map((d) => ({ label: (d.advertiser || "").slice(0, 10), value: d.count }))} />
              </div>
            </section>

            <section className="tpi-board">
              <div className="tpi-panel-head"><h3>Creative gallery</h3><span>{topAds.length} top ads (spy-score)</span></div>
              <div className="tas-gallery">
                {topAds.map((ad) => (
                  <article className="tas-ad-card" key={ad.id}>
                    <div className="tas-ad-thumb">
                      {ad.thumbnail_url ? (
                        <img src={ad.thumbnail_url} alt={ad.advertiser_name} referrerPolicy="no-referrer" loading="lazy" />
                      ) : (
                        <div className="tas-ad-thumb-fallback">{(ad.advertiser_name || "AD")[0]}</div>
                      )}
                      <span className="tas-ad-rank">#{ad.rank_position}</span>
                      {ad.is_active && <span className="mas-active-dot" title="Aktif">● live</span>}
                      {ad.video_url && (
                    ...
                    <div className="tas-ad-body">
                      ...
                      {ad.ai_enrichment?.isBrandedContent && (
                        <span className="mas-brand-badge">🤝 {ad.ai_enrichment.brandPartner}</span>
                      )}
                      <p>{ad.ai_enrichment?.summary || ad.title || "No copy"}</p>
                      ...
                      <div className="tpi-chip-group">
                        <span className="tpi-chip">{ad.ai_enrichment?.format}</span>
                        {(ad.ai_enrichment?.publisherPlatform || []).slice(0, 3).map((p) => (
                          <span className="tpi-chip tpi-chip-muted" key={p}>{p}</span>
                        ))}
                      </div>
                      {ad.link_url && (
                        <a href={ad.link_url} target="_blank" rel="noopener noreferrer" className="tpi-link">
            ...
            <section className="tpi-grid-main">
              <div className="tpi-panel">
                <div className="tpi-panel-head"><h3>Format mix</h3></div>
                <ClusterList rows={summary.formatMix || []} labelKey="format" valueKey="count" />
              </div>
              <div className="tpi-panel">
                <div className="tpi-panel-head"><h3>Platform mix</h3></div>
                <ClusterList rows={summary.platformMix || []} labelKey="platform" valueKey="count" />
              </div>
            </section>
            ...
            {(summary.partnerships || []).length > 0 && (
              <section className="tpi-panel">
                <div className="tpi-panel-head"><h3>Influencer partnerships</h3><span>{summary.partnershipCount} terdeteksi</span></div>
                <div className="tpi-chip-group">
                  {summary.partnerships.map((p, i) => (
                    <span className="tpi-chip" key={`${p.brand}-${i}`}>{p.brand} → {p.creator}</span>
                  ))}
                </div>
              </section>
            )}
            ...
            {(summary.spendAds || []).length > 0 && (
              <section className="tpi-panel">
                <div className="tpi-panel-head"><h3>Spend intel (iklan politik/sosial)</h3><span>{summary.spendAdsCount} ads</span></div>
                <div className="tpi-table-wrap">
                  <table className="tpi-table">
                    <thead><tr><th>Advertiser</th><th>Spend</th><th>Reach</th><th>Currency</th></tr></thead>
                    <tbody>
                      {summary.spendAds.map((s, i) => (
                        <tr key={i}>
                          <td>{s.advertiser}</td>
                          <td>{s.spendRaw || compactNumber(s.spendEstimate)}</td>
                          <td>{s.reachRaw || "-"}</td>
                          <td>{s.currency}</td>
                        </tr>
                      ))}
            ...
            <section className="tpi-panel">
              ...
              <div className="tpi-table-wrap">
                <table className="tpi-table">
                  <thead>
                    <tr><th>#</th><th>Advertiser</th><th>Format</th><th>Copy</th><th>CTA</th><th>Days</th><th>Active</th><th>Spy score</th></tr>
                  </thead>
                  <tbody>
                    {(reportDetail.items || []).map((ad) => (
                      <tr key={ad.id}>
                        ...
                        <td>{(ad.body_text || ad.title || "-").slice(0, 60)}</td>
                        <td>{ad.cta_type}</td>
