...
async function main() {
  ...
  try {
    ...
    if (!activitiesReady) {
      throw new Error("Activities table belum tersedia. Pastikan migration foundation sudah applied dulu.");
    }

    const sql = await readFile(
      new URL("../supabase/migrations/0025_crm_activity_bulk_phase4_phase5.sql", import.meta.url),
      "utf8",
    );

    console.log("Applying 0025_crm_activity_bulk_phase4_phase5.sql ...");
    await client.query(sql);
    console.log("CRM activity + bulk migration applied successfully.");
  } finally {
    await client.end();
  }
}
